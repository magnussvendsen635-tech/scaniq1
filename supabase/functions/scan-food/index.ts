        ...
        messages: [
          {
            ...
            content:
              ...
              "STEP 3b — FAT & DRESSING DETECTIVE (MANDATORY, advisory layer — runs INDEPENDENTLY of the addOil/addDressing toggles): " +
              "  • Always inspect the photo for SIGNS OF HIDDEN CALORIES: oily sheen/glossiness on meat or vegetables (cooking fat), visible pooled or smeared sauce/dressing/mayo/aioli/remoulade, dressing edges around lettuce, glossy coating on roasted veg, fryer crust, melted cheese, butter sheen. " +
              "  • Even the smallest visual cue MUST result in a realistic non-zero number in `hiddenSourcesEstimateKcal`. Do NOT default to 0 just because you are uncertain — a realistic estimate is always preferred over 0. Only return 0 when the dish is unambiguously dry/raw (e.g. a single raw apple, a glass of water). " +
              "  • `hiddenSourcesEstimateKcal` is ADVISORY ONLY and MUST NEVER be added into `calories`, `per100g`, or the actual macros — those still follow the toggle rules in step 2(f). " +
              "  • RECOGNISED BRAND/CHAIN CONTEXT: When you can identify a standardised fast-food / chain / packaged product (Big Mac, Whopper, Pringles, Snickers etc.), prefer the brand's published standardised calorie content over a pure visual estimate. " +
              "  • NOVA — be nuanced: only stamp NOVA 4 when there are clear industrial additives / packaged factory products. Fresh restaurant or homemade dishes should NOT be stamped NOVA 4 just because they look rich. Default DOWN if unsure. " +
              "  • OBJECTIVITY — always also return a realistic kcal RANGE for the portion in `caloriesMin` and `caloriesMax` (e.g. 650 and 800). The range MUST bracket the single `calories` value (caloriesMin ≤ calories ≤ caloriesMax) and reflect honest uncertainty (typically ±10–25%). " +
              "  • REASONING — populate `detectiveReasoning` with a short Danish sentence that explicitly states the hidden-source estimate and the visual evidence, e.g. 'Jeg har inkluderet 120 kcal for dressing/olie, da jeg ser olieglans på kødet og dressingkanter ved salaten.' If you estimated 0, briefly explain why (e.g. 'Tørt råt æble — ingen synlige fedtkilder.'). " +
